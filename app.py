...
app = Flask(__name__)
# CORS(app) 
CORS(app, expose_headers=["X-Graph-Exists", "X-Graph-Data"])
...
def predict():
    ...
    try:
        if "file" in request.files:
            ...
            print("Graph Base64 Length:", len(response.headers["X-Graph-Data"]))
            print("Graph Data Sent in Header")
...
def get_distribution_graph(data):
    ...
    tags.plot(
        ...
        wedgeprops=wp,
        explode=explode,
        ax=ax  # Add ax to specify the axes
    )
    ...
    graph.seek(0)  # Rewind the BytesIO object to the beginning
